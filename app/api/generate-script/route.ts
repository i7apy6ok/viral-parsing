...
function buildPrompt(
  ...
  const targetWords = durationMin ? Math.round(durationMin * 130 * 1.2) : 1300;
  const targetMin = durationMin ? Math.round(durationMin * 1.2) : 15;
  const variance = Math.floor(Math.random() * 10) - 5;
  const finalWords = targetWords + Math.round((targetWords * variance) / 100);

  const formatInstruction = isLong
    ? `Создай подробный сценарий для YouTube видео.
ДЛИНА ОРИГИНАЛА: ${durationMin ? durationMin + " минут" : "длинное видео"}.
ЦЕЛЕВАЯ ДЛИНА: примерно ${targetMin} минут (на 15-25% длиннее оригинала) — около ${finalWords} слов живой речи.
Пиши свободно и естественно, как живой рассказчик — не копируй структуру оригинала дословно, добавляй свои переходы, акценты и детали. Это должно звучать как уникальный авторский материал, а не пересказ.`
...
ВАЖНО: хуки должны содержать ТОЛЬКО spoken text — то что говорит человек на камеру.
НЕ добавляй описания видеоряда, ремарки про "кадры", "показать", "сцены" внутрь текста хука.
Визуальные описания — только в поле visualHook, не в hooks[].

2. ОСНОВНАЯ ЧАСТЬ (${durationMin ? `${Math.round(durationMin * 1.1)}-${Math.round(durationMin * 1.3)}` : "8-15"} минут):
Подробное раскрытие темы с примерами, историями, фактами. Несколько логических блоков с переходами. Каждый блок — отдельный абзац.
...
Разбей ВСЁ содержимое body на отдельные предложения — каждый элемент массива = одно предложение из текста.
Не объединяй несколько предложений в один элемент и не дроби одно предложение на части.
Целевое количество sentences примерно ${Math.round(finalWords / 15)} штук.`
